...
built. A pod keeps one way of obtaining the token for its whole life: in `auto` mode the first login decides, and a
failure of a later login does not switch the way back.

| Property name                                       | Type                          | Default                              | Read when                                                    |
|-----------------------------------------------------|-------------------------------|--------------------------------------|--------------------------------------------------------------|
| `spring.cloud.consul.config.login.mode`             | `auto`, `kubernetes` or `m2m` | `auto`                               | ConfigData phase and `TokenStorage` bean creation            |
...
In `auto` mode the `kubernetes` way is tried first. If it fails, the pod falls back to the `m2m` way and logs one
