...
* `Features`
  - HTTP calls to maas-agent are now retried on retryable status codes, not only on `IOException`.
    Retryable: 5xx, 429, **405** and **401**. See "Retry behaviour and configuration" in README for why
    the two 4xx codes are included — without them the client does not survive a Postgres leader switchover.
  ...
  - New configuration: `maas.http.retry.max-total-duration-ms` (`60s` by default) — a single
    ...
    it, so there are no separate knobs to keep consistent.
